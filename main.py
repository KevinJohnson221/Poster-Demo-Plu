...
def posters():
    ...
    if conn:
        cur = conn.cursor()
        all_posters = cur.execute('SELECT * FROM posters LEFT JOIN scores s on posters.poster_id=s.poster_id WHERE s.poster_id IS NULL').fetchall()
        conn.close()
...
def rate_poster(posterID):
    if request.method == 'GET':
        conn = poster_db()
        cur = conn.cursor()
        poster = cur.execute('SELECT * FROM posters WHERE poster_id = ?', (posterID,)).fetchone()
        conn.close()

        filename = str(posterID) + "." + poster['img_ext']
        path = os.path.join('..', '..', 'static', 'posters')
        file = os.path.join(path, filename)
        print(file)

        print('displaying poster with id:', posterID)
        return render_template('judge_poster.html', poster=poster, image=file)
    else:
        conn = poster_db()
...
def upload_poster():
    ...
    if request.method == "GET":    
        ...
        if poster_image.filename.endswith('.jpg'):
            type = "jpg"
        else:
            type = "png"
        ...
        if conn:
            cur = conn.cursor()
            cur.execute("""INSERT INTO posters (poster_title, poster_emails, poster_category, poster_description, img_ext) VALUES (?, ?, ?, ?, ?)""",
                        (poster_title, poster_emails, poster_category, poster_description, type))
            ...
            if poster:
                ...
                name = str(poster[0]) + "." + type
                print(name)
                filename = secure_filename(name)
                poster_image.save(os.path.join(app.root_path, app.config['UPLOAD_FOLDER'], filename))
