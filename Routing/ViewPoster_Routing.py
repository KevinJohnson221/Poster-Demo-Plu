...
@ViewPoster_Routing.route('/view_poster/<int:poster_id>', endpoint="view_poster")
def load_poster(poster_id):
   ...
   try:
      ...
      ext = dbDriver.get_image_extension()


      filename = str(poster_id) + "." + ext
   ...
   return render_template('View_Poster.html', id=poster_id, title=poster_title, emails=poster_emails, category=poster_category, description=poster_description, clarity_rating=clarity, organization_rating=organization, content_rating=content, relevance_rating=relevance, visuals_rating=visual, total_rating=total, image=file)
